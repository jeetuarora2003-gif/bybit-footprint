...
// FIX #4: ROW_SIZE matches backend rowSize = 1.0 (standard BTC tick)
const ROW_SIZE     = 1.0;
...
export default function ChartCanvas({ candles, settings, activeFeatures, onCrosshairMove }) {
  ...
  const stateRef = useRef({
    ...
    velocityX: 0,
    lastDragTime: 0,
    // FIX #7: track autoScroll for button visibility
    showGoLive: false,
    // FIX #8: last hovered candle for crosshair callback
    hoveredCandle: null,
    // FIX #7 (canvas resize): cache last known size to avoid resize every frame
    lastCanvasW: 0,
    lastCanvasH: 0,
  });

  // FIX #7: "Go Live" button state in React so it re-renders correctly
  const [showGoLive, setShowGoLiveState] = useRef ? 
    // eslint-disable-next-line react-hooks/rules-of-hooks
    (() => { const [s, ss] = [useRef(false), useRef(null)]; return [s, ss]; })() 
    : [{ current: false }, { current: null }];

  // We use a simple external div approach — track via a ref flag updated each frame
  ...
  useEffect(() => {
    ...
    const loop = () => {
      if (!running) return;
      const prevGoLive = stateRef.current.showGoLive;
      drawFrame(
        canvasRef.current,
      ...
      // FIX #8: show/hide go-live button based on autoScroll state
      const btn = goLiveBtnRef.current;
      if (btn) {
        btn.style.display = stateRef.current.autoScroll ? "none" : "flex";
      }
  ...
  const onMove = useCallback((e) => {
    ...
    // FIX #8: fire crosshair callback with hovered candle data
  ...
  return (
    <div className="chart-canvas-container" ref={containerRef} style={{ position: "relative" }}>
      ...
      />
      {/* FIX #8: Go Live button — shown when user pans away from latest bar */}
      <button
        ref={goLiveBtnRef}
        style={{
          display: "none",
...
function drawFrame(canvas, container, state, candles, settings, activeFeatures, crosshairCbRef) {
  ...
  // FIX #7: Only resize canvas when dimensions actually change (avoids expensive clear every frame)
  ...
  // FIX #3: VWAP resets at each UTC midnight (session-accurate)
  ...
  // FIX #8: find hovered candle for crosshair callback
  ...
  for (const cl of clusters) {
    ...
    if (rH >= minFont && candleW >= 20 && settings.dataView !== "none") {
      ...
      if (dv === "volume") {
        ...
        ctx.fillStyle = "#666"; ctx.textAlign = "center"; ctx.fillText("×", cx, yM);
...
function drawDOM(ctx, bids, asks, chartW, chartH, p2y) {
  ...
  for (const a of asks) {
    ...
    const bw = (a.size / maxS) * maxBarW;
    ctx.fillRect(chartW - bw, y - Math.max(1, rh / 2), bw, h);
  }
